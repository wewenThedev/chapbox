...
class CustomAppBarWithBack extends StatefulWidget implements PreferredSizeWidget{
  final String title;
  final double height = 100.0;
  final GlobalKey<ScaffoldState> scaffoldKey;

  // Le constructeur pour accepter le titre et la clé du Scaffold pour ouvrir le Drawer
  const CustomAppBarWithBack({super.key, required this.title, /*this.height = kToolbarHeight,*/ required this.scaffoldKey});
...
class _CustomAppBarWithBackState extends State<CustomAppBarWithBack>{
  ...
  Widget build(BuildContext context) {
      return AppBar(
      leading: IconButton(
        icon: Icon(/*Iconsax.arrow_left*/ Iconsax.arrow_square_left_copy, color: appGrey, size: 35.0),
        onPressed: () { // back
      ...
      actions: [
        IconButton(
          icon: Icon(
            Iconsax.notification /*Iconsax.notification_bing*/  /*Icons.notifications*/, 
            color: appGrey,
            size: 35.0
          ...
          onPressed: () {
            ...
            Navigator.push(context, MaterialPageRoute(builder: (context) => NotificationsListScreen()) );
          },
        ),
      ],
    );
